...
docker compose --env-file .env -f deploy/compose/compose.yaml exec -T postgres \
  sh -c 'for migration in 000008_offer_category.sql 000009_user_preference_weights.sql; do psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" -f "/docker-entrypoint-initdb.d/$migration"; done'
```
